...
from .models import Wallet, Transaction, PaymentMethod, PaymentIntent, OrderPayment, FraudCheck, Payout
from .serializers import (
    WalletSerializer, TransactionSerializer, DepositSerializer,
    WithdrawSerializer, PaymentMethodSerializer, AddPaymentMethodSerializer,
    PaymentIntentSerializer, OrderPaymentSerializer, PayoutSerializer
)
from .services import WalletService, PaymentProcessor, FraudDetectionService, IdempotencyService, EmailService


def wallet_locked_response(wallet):
    if not wallet.is_active:
        return Response({'error': 'wallet_inactive', 'message': 'This wallet is not active'}, status=status.HTTP_403_FORBIDDEN)
    if wallet.is_locked():
        return Response({
            'error': 'wallet_locked',
...
def deposit(request):
    ...
    if not result['success']:
        ...
        return Response({'error': error_code, 'message': result['error']}, status=status.HTTP_400_BAD_REQUEST)
    ...
    try:
        with db_transaction.atomic():
            transaction_obj = WalletService.debit(
                ...
                metadata={'account_details': serializer.validated_data['account_details']}
            )
