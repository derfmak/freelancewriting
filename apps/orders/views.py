...
def dashboard_stats(request):
    ...
    stats = {
        ...
        'in_progress_orders': orders.filter(status='ongoing').count(),
        ...
        'completion_rate': 0,
        'overdue_orders': orders.filter(deadline__lt=timezone.now(), status__in=['pending', 'ongoing']).count(),
        'unread_messages': 0,
    }
...
def list_orders(request):
    ...
    if status_filter == 'revision':
        orders = orders.filter(status='ongoing', revision_count__gt=0)
    elif status_filter == 'in_progress':
        orders = orders.filter(status='ongoing', revision_count=0)
    elif status_filter in FRONTEND_TO_MODEL_STATUS:
        orders = orders.filter(status=FRONTEND_TO_MODEL_STATUS[status_filter])
    elif status_filter:
...
def pending_orders(request):
    ...
    orders = Order.objects.filter(status='pending').select_related('student').order_by('deadline')
...
def active_orders(request):
    ...
    orders = Order.objects.filter(status='ongoing').select_related('student').order_by('deadline')
    serializer = OrderAdminSerializer(orders, many=True)
    return Response(serializer.data)
...
def approve_order(request, order_id):
    ...
    return Response({'message': 'Order approved and started'})
...
def complete_order(request, order_id):
    ...
    order = get_object_or_404(Order, id=order_id, status='awaiting_review')
