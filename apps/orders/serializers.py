...
class OrderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'is_late', 'auto_cancel_at', 'deadline_extended_at', 'deadline_extension_count',
            'can_cancel', 'can_edit', 'can_resubmit', 'can_reorder', 'can_split',
            'timeline'
        ]
        read_only_fields = [
            'id', 'order_number', 'client', 'created_at', 'updated_at',
            'last_activity_at', 'secure_links', 'is_late', 'auto_cancel_at',
            'deadline_extended_at', 'deadline_extension_count'
        ]
    ...
    def validate_new_deadline(self, value):
        if value < timezone.now() + timezone.timedelta(hours=12):
            raise serializers.ValidationError(
                'New deadline must be at least 12 hours from now.'
            )
        return value